...
import { Field, arrayInsert, arrayRemove } from 'redux-form' // Não esquecer os {} ao instanciar as Bibliotecas
    ...
    renderRows() {
        ...
        return list.map((item, index) => (
            <tr key={index}>
                ...
                <td>
                    <BillingCycleButton btn='success' icon='plus' type='button' action={() => this.add(index + 1)} />
                    <BillingCycleButton btn='warning' icon='clone' type='button' action={() => this.add(index + 1, item)} />
                    <BillingCycleButton btn='danger' icon='trash-o' type='button' action={() => this.remove(index)} />
                </td>
            </tr>
        ))
    }
